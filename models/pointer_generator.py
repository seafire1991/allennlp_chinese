...
class PointerGenerator(Model):
    ...
    def forward(self,  # type: ignore
        ...
        for timestep in range(num_decoding_steps):
            ...
            z = torch.ones([batch_size, 1, self._decoder_hidden_dim])
            coverage = torch.zeros([batch_size, self._max_decoding_steps])
            decoder_output, decoder_hidden, z, attn, coverage = self.decoder(decoder_input, decoder_hidden, encoder_outputs, z, input_choices, coverage)
            ...
            if random.randint(1, 10) > 5:
                ...
                decoder_input = targets[:, timestep].view(batch_size, -1)
