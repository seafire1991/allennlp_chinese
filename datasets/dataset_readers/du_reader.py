...
@DatasetReader.register("dureader")
class DuReader(DatasetReader):
    """
    ...
    def _read(self, file_path: str):
        ...
        with open(file_path, "r", encoding="utf-8") as dataset_file:
            ...
            for article in dataset:
                paragraph_json = article["documents"][article["answer_docs"][0]] if "answer_docs" in article and len(article["answer_docs"]) > 0 else 0
                if paragraph_json == 0:continue
                tokenized_paragraph = self._tokenizer.tokenize(" ".join(paragraph_json["segmented_paragraphs"][paragraph_json["most_related_para"]]))
                ...
                span_starts = [article["answer_spans"][0][0]]
                span_ends = [article["answer_spans"][0][1]]
