...
from bita.nlp.apis.nlp import segment


@Predictor.register("c-srl")
class SemanticRoleLabelerPredictor(Predictor):
    ...
    def predict_tokenized(self, tokenized_sentence: List[str]) -> JsonDict:
        ...
        A dictionary representation of the semantic roles in the sentence.
        """
        instances = self.tokens_to_instances(tokenized_sentence)
        if not instances:
            return sanitize({"verbs": [], "words": tokenized_sentence})
    ...
    def make_srl_string(words: List[str], tags: List[str]) -> str:
        ...
        for (token, tag) in zip(words, tags):
            if tag.startswith("I-") or tag.startswith("E-"):
                ...
                elif tag == "O" or tag.startswith("S-"):
                    frame.append(token)
    ...
    def tokens_to_instances(self, tokens):
        tokens = [word_pos.split("/") for word_pos in segment(tokens, ispos=True).split(" ")]
        ...
        for i, word in enumerate(tokens):
            if word[1] == "VV":
                ...
                verb_labels[i] = 1
                new_tokens = [Token(token[0]) for token in tokens]
                instance = self._dataset_reader.text_to_instance(new_tokens, verb_labels)
    ...
    def _sentence_to_srl_instances(self, json_dict: JsonDict) -> List[Instance]:
        ...
        return self.tokens_to_instances(sentence)
